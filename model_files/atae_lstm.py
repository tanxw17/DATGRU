...
class ATAE_LSTM(nn.Module): 
    def __init__(self, args):
        ...
        self.lstm =nn.LSTM(input_size = word_embed_dim+args.aspect_embed_dim,
        ...
        self.w = Parameter(rand_init(1, hidden_size + aspect_embed_dim))
    ...
    def forward(self, content, aspect, mask):
        ...
        aspect_embedding = self.aspect_embed(aspect)
        aspect_embedding = aspect_embedding.expand(-1, content.size()[0], -1)
        aspect_embedding = aspect_embedding.permute(1,0,2)
        content = torch.cat((content, aspect_embedding), dim=2)
