...
def train(train_iter, test_iter, dev_iter, model, args, text_field, aspect_field, sm_field, predict_iter):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        dev_acc, _, _, _ = eval(dev_iter, model, args)
        if args.early > 0:
            if dev_acc > b_acc:
                early = 0
                b_acc = dev_acc
            ...
            b_acc = dev_acc
    test_acc, _, _, _ = eval(test_iter, model, args, True)
    if args.verbose == 1:
...
def eval(data_iter, model, args, conf=False):
    ...
    return accuracy, pooling_input, relu_weights, res_dict    # what is relu weights
